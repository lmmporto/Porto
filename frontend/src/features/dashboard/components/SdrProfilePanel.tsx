...
const getPriorityContent = (gaps: Record<string, number> | undefined, insights: any[] | undefined) => {
  if (!gaps || Object.keys(gaps).length === 0) {
    return {
      ...
      targetBehavior: 'Aplicar melhores práticas',
      playbook: [
      ...
      behavioralSignal: 'Consistência na execução.',
      executiveSummary: 'Aguardando mais análises para gerar um resumo executivo.'
    };
  }

  const topGapEntry = Object.entries(gaps).reduce((a, b) => (b[1] > a[1] ? b : a), ['', 0]);
  const topGap = topGapEntry[0];

  const cognitiveInsight = insights?.find(i => i.label === 'Ponto Cego Identificado')?.value || `O SDR tende a ${topGap.toLowerCase()} em algumas chamadas.`;
  const behavioralSignal = insights?.find(i => i.label === 'Sinal Comportamental')?.value || `Foco em ${topGap.toLowerCase()}.`;
  const executiveSummary = insights?.find(i => i.label === 'Resumo Executivo')?.value || `O principal ponto de melhoria é ${topGap.toLowerCase()}.`;


  switch (topGap) {
    case 'Pitch Prematuro':
      return {
        mainGoal: 'Segurar a Demo',
        targetBehavior: 'Explorar Implicação',
        playbook: [
          { title: '1. Valide a dor', description: 'Confirme o problema antes de apresentar.' },
          { title: '2. Confirme o impacto', description: 'Quantifique o custo do problema.' },
          { title: '3. Conecte valor à dor', description: 'Apresente a solução como resposta direta.' },
          { title: '4. Feche o próximo passo', description: 'Convite natural para a demo.' },
        ],
        cognitiveInsight,
        behavioralSignal,
        executiveSummary
    ...
    case 'Urgência Fraca':
      return {
        mainGoal: 'Criar Urgência',
        targetBehavior: 'Aprofundar Consequências',
        playbook: [
          { title: '1. "O que acontece se nada mudar?"', description: 'Explore o custo da inação.' },
          { title: '2. Conecte com metas', description: 'Como o problema afeta os KPIs do lead?' },
          { title: '3. Ancore no tempo', description: 'Pergunte sobre prazos e metas futuras.' },
          { title: '4. Apresente o "depois"', description: 'Mostre o cenário ideal com a solução.' },
        ],
        cognitiveInsight,
        behavioralSignal,
        executiveSummary
    ...
    case 'Salto de Etapa':
      return {
        mainGoal: 'Respeitar o Processo',
        targetBehavior: 'Seguir o SPIN Selling',
        playbook: [
          { title: '1. Não pule etapas', description: 'Siga a sequência de perguntas SPIN.' },
          { title: '2. Valide cada fase', description: 'Confirme o entendimento do lead antes de avançar.' },
          { title: '3. Construa valor gradualmente', description: 'Não apresente a solução cedo demais.' },
          { title: '4. Pratique a escuta ativa', description: 'Deixe o lead falar e absorva as informações.' },
        ],
        cognitiveInsight,
        behavioralSignal,
        executiveSummary
    ...
    default:
      return {
        mainGoal: `Corrigir: ${topGap}`,
        targetBehavior: 'Revisar calls com este gap',
        playbook: [
          { title: '1. Identifique o padrão', description: 'Veja em que momento este gap ocorre.' },
          { title: '2. Estude o playbook', description: 'Revise as melhores práticas para este ponto.' },
        ],
        cognitiveInsight,
        behavioralSignal,
        executiveSummary
      ...
      <div className="mx-auto max-w-[1460px] px-4 py-4 sm:px-6 lg:px-8 w-full h-auto">
        ...
        <section className="mt-5 grid grid-cols-2 gap-4 xl:grid-cols-4">
          <article className="kpi-card rounded-[20px] p-5">
            ...
            <div className="metric-value mt-4 text-[42px] font-semibold text-green2">{sdrData.nota_spin?.toFixed(1) || '0.0'}</div>
          </article>
            ...
            <article className="glass rounded-[24px] p-5 md:p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="mt-2 text-[24px] font-semibold tracking-[-0.03em] text-white">
                    {priorityContent.playbook[0]?.title || 'Playbook Genérico'}
                  </h2>
                </div>
              </div>
        ...
        <section className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-2">
            ...
            <div className="mt-6 space-y-5">
              <div>
                ...
                <div className="progress-track">
                  <div className="progress-fill-purple h-full rounded-full" style={{ width: `${Math.round((sdrData.media_dor || 0) * 10)}%` }}></div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="progress-track">
                  <div className="progress-fill-green h-full rounded-full" style={{ width: `${Math.round((sdrData.media_dominio || 0) * 10)}%` }}></div>
                </div>
              </div>
          ...
          <article className="glass rounded-[24px] p-5 md:p-6">
            ...
            <div className="mt-6 space-y-5">
              {sdrData.recurrent_gaps && Object.entries(sdrData.recurrent_gaps).length > 0 ? Object.entries(sdrData.recurrent_gaps).map(([gap, count]) => (
                <div key={gap}>
        ...
        <section className="mt-5 grid grid-cols-1 gap-5 xl:grid-cols-2">
          ...
          <article className="summary-card rounded-[24px] border border-white/8 p-5 md:p-6 shadow-soft">
            <div className="flex items-center gap-3">
              <span className="section-dot"></span>
              <div>
                <div className="tiny-caps text-[11px] font-semibold text-white/40">6. Resumo final</div>
                <h2 className="mt-2 text-[24px] font-semibold tracking-[-0.03em] text-white">
                  Resumo Executivo
                </h2>
              </div>
            ...
            <p className="mt-6 text-[14px] leading-8 text-white/68">
              {priorityContent.executiveSummary}
            </p>
          </article>
        </section>

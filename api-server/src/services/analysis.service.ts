...
export async function transcribeRecordingFromHubSpot(
  ...
  try {
    const audioResponse = await axios.get<ArrayBuffer | Buffer>(call.recordingUrl, {
    ...
    const rawData = audioResponse.data;
    ...
    localFilePath = path.join(
      os.tmpdir(),
      `call-${randomUUID()}.${detectAudioExtension(contentType)}`,
    );
    ...
    if (!isTranscriptionResult(parsed)) {
      throw new Error('Resposta de transcrição inválida recebida do Gemini.');
    }
...
export async function analyzeCallWithGemini(
  ...
  if (
    ...
    console.log(
      `✅ [Short-circuit] Análise recuperada do cache: ${CURRENT_ANALYSIS_VERSION}`,
    );
